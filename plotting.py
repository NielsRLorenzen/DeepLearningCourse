...
def plot_2d_latents(ax, qz, z, y, cmap=None):
    z = z.to('cpu')
    ...
    if cmap:
        colors = [palette[cmap[l]] for l in y]
        legend_elements =  [Patch(facecolor=palette[cmap[l]],label=l) for l in set(y)]
    else:
        colors = [palette[l] for l in y]
        legend_elements =  [Patch(facecolor=palette[l],label='{}'.format(l)) for l in set(y)]
...
def plot_latents(ax, z, y,cmap=None):
    z = z.to('cpu')
    palette = sns.color_palette()
    if cmap:
        colors = [palette[cmap[l]] for l in y]
...
def make_vae_plots(vae, x, y, outputs, training_data, validation_data, cmap=None, tmp_img="tmp_vae_out.png", figsize=(18, 18)):
